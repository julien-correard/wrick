...
U8 game_drewpf = FALSE;  /* current tick drew the playfield (interpolable) */
...
game_speed(S8 delta)
{
  ...
  if ((U8)p != game_period) {
    game_period = (U8)p;
    sys_printf("xrick/game: %d ms per tick\n", game_period);
  }
}
 ...
 * Main loop
 *
 * The game logic runs at its original pace (game_period per tick).
 * Presentation runs at ~60hz: when the current tick drew the playfield,
 * intermediate frames are rendered with entity positions interpolated
 * between the pre-tick snapshot and the post-tick state, and with a
 * fractional vertical offset while scrolling. This gives smooth motion
 * without altering the game speed in any way.
 */
#define GAME_RENDER_PERIOD 16  /* presentation period (ms), ~60fps */

static void
game_render(U32 a16, S16 off)
{
...
game_run(void)
{
  U32 tm, now, el, acc = 0, lastr;
  U32 a16;
  S16 off;
  U8 st_before = 0, st_after = 0;
	...
	while (game_state != EXIT) {
		...
		tm = now;
		if (el > 4 * game_period)
			el = 4 * game_period;  /* don't fast-forward after stalls */
		acc += el;
...
ent_snap();          /* save pre-tick state for interpolation */
		...
		a16 = (acc < game_period)
		  ? (U32)(((U64)acc * 65536) / game_period)
		  : 65536;

		/* presentation */
		if (!game_drewpf || game_waitevt) {
			/* menus, intros, pauses: present the tick's dirty rects */
			sysvid_update(game_rects);
			draw_STATUSRECT.next = NULL;  /* FIXME freerects should handle this */
		}
		else {
			/* playing: interpolated frame at ~60hz */
			off = 0;
			if (st_before == SCROLL_UP && st_after == SCROLL_UP)
				off = (S16)((((U64)65536 - a16) * 8) >> 16);
			else if (st_before == SCROLL_DOWN && st_after == SCROLL_DOWN)
				off = -(S16)((((U64)65536 - a16) * 8) >> 16);
			game_render(a16, off);
		}

		/* pace rendering to ~60hz */
		now = sys_gettime();
		el = now - lastr;
		if (el < GAME_RENDER_PERIOD)
			sys_sleep(GAME_RENDER_PERIOD - el);
		lastr += GAME_RENDER_PERIOD;
		if (lastr < now || lastr > now + GAME_RENDER_PERIOD)
			lastr = now;  /* resync after stall */
	}
...
play3(void)
{
  ...
  /* sound */
  draw_drawStatus();   /* draw the status bar onto the buffer*/
  game_drewpf = TRUE;  /* this tick can be interpolated */
