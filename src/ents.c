...
/*
 * Interpolated rendering support
 *
 * The game logic ticks at its own (slow) pace. To get smooth motion,
 * intermediate frames are rendered between logic ticks, with entity
 * positions linearly interpolated between the state at the beginning
 * of the tick (snapshot) and the current state.
 */
static S16 isnap_x[ENT_ENTSNUM + 1];
static S16 isnap_y[ENT_ENTSNUM + 1];
static U8 isnap_n[ENT_ENTSNUM + 1];

/*
 * Snapshot entity positions (call before each game logic tick)
 */
void
ent_snap(void)
{
  ...
  for (i = 0; i < ENT_ENTSNUM + 1 && ent_ents[i].n != 0xff; i++) {
    isnap_x[i] = ent_ents[i].x;
    isnap_y[i] = ent_ents[i].y;
    isnap_n[i] = ent_ents[i].n;
  }
}

/*
 * Draw all entities onto the frame buffer, with positions interpolated
 * between the pre-tick snapshot and the current positions.
 *
...
ent_draw_interp(U32 a16, S16 off)
{
  ...
  for (i = 0; i < ENT_ENTSNUM + 1 && ent_ents[i].n != 0xff; i++) {
...
#ifdef ENABLE_CHEATS
    if (!ent_ents[i].sprite && !game_cheat3)
#else
    if (!ent_ents[i].sprite)
#endif
      continue;  /* nothing to draw */

    dx = ent_ents[i].x - isnap_x[i];
    dy = ent_ents[i].y - isnap_y[i];

    /* interpolate only when the entity existed before the tick and
    ...
    if (!isnap_n[i] || dx > 0x20 || dx < -0x20 || dy > 0x16 || dy < -0x16) {
      ...
      yi = isnap_y[i] + (S16)(((S32)dy * (S32)a16 + 0x8000) >> 16);
    }

    draw_sprite2(ent_ents[i].sprite, (U16)xi, (U16)yi, ent_ents[i].front);
  }
}
